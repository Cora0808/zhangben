...
const catInfo=k=>CAT_MAP[k]||{n:'其他',e:'✨',g:'day'};
/* 收入来源：全部到账=活钱 */
const INCS=[
...
function bootData(){
  ...
  const b=defaults();
  S=Object.assign(b,d,{recs:Array.isArray(d.recs)?d.recs:[],plans:Array.isArray(d.plans)?d.plans:[],goals:Array.isArray(d.goals)&&d.goals.length?d.goals:b.goals,smart:d.smart||{},tmpl:Array.isArray(d.tmpl)?d.tmpl:[],stateFlags:d.stateFlags||{},customHol:Array.isArray(d.customHol)?d.customHol:[],customWork:Array.isArray(d.customWork)?d.customWork:[]});
...
function accVal(k){
  ...
  if(k==='gold'){ /* 市值型：用期初/最近更新市值 + 无自动变动（手动更新） */
    let v=S.t0.acc.gold_mv||0;
    S.recs.forEach(r=>{ if(r.type==='mvup'&&r.acc==='gold'){ v=+r.amt||0; } });
    return v;
  }
  return 0;
}
...
function assets(){
  ...
  return {cash,ylb,gold,save,total:Math.round((cash+ylb+gold+save)*100)/100};
}
...
function periodSpent(recs){ /* 我自己出的消费 + 给对象 + 转出? 转出不算花——只算支出类 */
  return Math.round(recs.filter(r=>r.type==='exp'||r.type==='obj'?false:false).length?0:recs.filter(r=>r.type==='exp').reduce((s,r)=>s+(+r.amt||0),0)*100)/100;
}
...
function nextPayday(){ /* 从今天往后的下一个发薪日 */
...
function coachDigest(){
  ...
  lines.push('资产: 活钱 ¥'+fmt(a.cash)+' · 余利宝 ¥'+fmt(a.ylb)+' · 黄金 ¥'+fmt(a.gold)+' · 代存 ¥'+fmt(a.save)+' · 合计 ¥'+fmt(a.total));
...
function checkUpdate(){
  try{
    fetch('./version.json?v='+Date.now()).then(r=>r.ok?r.json():null).then(j=>{
      if(!j||!j.ver) return;
      const curV=(window.APP_VER_CUR||APP_VER);
      if(j.ver!==curV&&j.ver>curV){ showToast('发现新版 '+j.ver+'，下拉刷新即更新','warn'); }
    }).catch(()=>{});
  }catch(e){}
}
/* ---------- 备份导出/导入 ---------- */
...
function openSheet(){ $('mask2').classList.add('on'); $('sheet').classList.add('on'); }
function closeSheet(){ $('mask2').classList.remove('on'); $('sheet').classList.remove('on'); }
...
/* ================= 记一笔 ================= */
const REC={mode:'exp', amt:'', cat:'food', src:'salary', payer:'me', mvDir:'in', mvTo:'ylb', goal:'car', note:'', date:''};
...
function renderPickArea(){
  ...
  let h='';
  if(REC.mode==='exp'){
    /* 常用排前：优先智能记忆分类的快捷 chips */
    const hint=smartGuess(REC.note);
    h+=CATS.map(c=>'<div class="chip '+(REC.cat===c.k?'on':'')+'" style="'+(REC.cat===c.k?'background:'+cCol(c.k):'')+'" onclick="pickCat(\''+c.k+'\')">'+c.e+''+c.n+'</div>').join('');
    h+='<div style="display:flex;gap:6px;margin-top:8px;align-items:center"><span style="font-size:11px;color:var(--txt3)">钱从谁出：</span>'+
       '<div class="chip '+(REC.payer==='me'?'on':'')+'" style="'+(REC.payer==='me'?'background:var(--cy);color:#04121a;font-weight:700':'')+'" onclick="REC.payer=\'me\';renderPickArea()">我自己</div>'+
       '<div class="chip '+(REC.payer==='obj'?'on':'')+'" style="'+(REC.payer==='obj'?'background:var(--pu);color:#fff;font-weight:700':'')+'" onclick="REC.payer=\'obj\';renderPickArea()">对象亲密付(她付)</div></div>';
  }
    ...
    const targets=[{v:'ylb',n:'余利宝'},{v:'gold',n:'黄金'},{v:'car',n:'买车基金'},{v:'sav',n:'其他存钱'}];
    ...
    h+='<div style="font-size:11px;color:var(--txt3);margin-top:7px">'+(REC.mvDir==='in'?'钱从我活钱转入：记录投资/存钱':'钱回到我活钱（本金回来，不算收入）')+'</div>';
  }
  pa.innerHTML=h;
}
...
function saveRec(){
  ...
  if(REC.mode==='exp'){
    ...
    else { rec.type='exp'; rec.cat=REC.cat; if(note) smartLearn(note,REC.cat); }
  }
...
function coachFeed(){
  ...
  const inc=periodIn(cyc), exp=cyc.filter(r=>r.type==='exp').reduce((s,r)=>s+(+r.amt||0),0);
  /* 1) 发薪日提醒 */
  ...
  if(toNext>0){
    ...
    if(actualDaily>targetDaily*1.15&&targetDaily>=0){
      /* 找超速大头 */
      const byCat={}; cyc.filter(r=>r.type==='exp').forEach(r=>{ byCat[r.cat]=(byCat[r.cat]||0)+(+r.amt||0); });
  ...
  if(car&&car.target){ const v=saveVal('car')+(S.t0.save.car||0); const pct=Math.round(v/car.target*100);
...
function renderHome(){
  ...
  h+='<div class="card gr-b"><div class="c-head"><div class="c-title">FUND / 资金</div><div class="c-sub">'+cycleLabel()+' · 距发薪 '+toNext+' 天</div></div>'+
     ...
     '<div class="assets">'+assetChip('活钱',money(a.cash),'gr')+assetChip('余利宝',money(a.ylb),'cy')+assetChip('黄金',money(a.gold),'ye')+assetChip('代存',money(a.save),'pu')+assetChip('净资产',money(a.total),'','total')+'</div></div>';
  /* 本周期到账/支出 */
...
function recRow(r){
  ...
  return '<div class="row"><div class="ic" style="background:rgba(255,255,255,.05)">'+ic+'</div><div class="mid"><div class="t1">'+esc(t1)+'</div><div class="t2">'+esc(t2)+'</div></div><div class="amt '+(r.type==='inc'||r.type==='sv'&&r.dir==='out'||r.type==='mv'&&r.dir==='out'?'':'mg')+'" style="color:'+c+'">'+(r.type==='inc'||(r.type==='mv'&&r.dir==='out')||(r.type==='sv'&&r.dir==='out')?'+':'−')+fmt(r.amt)+'</div></div>';
}
...
function renderCycle(){
  ...
  const exps=cyc.filter(r=>r.type==='exp');
  ...
  h+='<div class="card cy-b"><div class="c-head"><div class="c-title">CYCLE / 发薪周期</div><div class="c-sub">'+cycleLabel()+'</div></div>'+
     ...
     '<div style="font-size:12px;color:var(--txt3);margin-top:8px">到账 <b style="color:var(--gr)">'+money(inc)+'</b> · 支出 <b style="color:var(--mg)">'+money(exp)+'</b>'+(objs?' · 对象付 <b style="color:var(--pu)">'+money(objs)+'</b>':'')+'</div></div>';
  h+='<div class="card"><div class="c-head"><div class="c-title">SPEND / 花在哪</div></div>';
  if(!sorted.length) h+='<div class="empty">这个周期还没有支出</div>';
  else sorted.slice(0,6).forEach(k=>{ const v=byCat[k], p=maxV?Math.round(v/maxV*100):0, ci=catInfo(k); h+='<div style="display:flex;align-items:center;gap:8px;margin-bottom:7px"><span style="width:70px;font-size:12px;flex-shrink:0">'+ci.e+' '+ci.n+'</span><div class="bar" style="flex:1"><i style="width:'+p+'%;background:'+cCol(k)+'"></i></div><span style="font-family:var(--mono);font-size:12px">'+money(v)+'</span></div>'; });
...
function renderGoal(){
  ...
  (S.goals||[]).forEach(g=>{
    const v=saveVal(g.id)+(S.t0.save[g.id]||0);
...
function planForm(id){
  ...
  openModal('<div class="mh">'+(p?'编辑':'新增')+'计划项</div>'+
    ...
    '<div class="field"><div class="fl">支出分类（类型选"支出"时）</div><select class="inp" id="pl_cat">'+CATS.map(c=>'<option value="'+c.k+'"'+(cur.cat===c.k?' selected':'')+'>'+c.e+c.n+'</option>').join('')+'</select></div>'+
...
function savePlan(id){
  ...
  save(); closeModal(); render(); showToast('已保存','ok');
}
function delPlan(id){ if(!confirm('删除这个计划？')) return; S.plans=(S.plans||[]).filter(x=>x.id!==id); save(); closeModal(); render(); }
...
function importModal(){ openModal('<div class="mh">恢复备份</div><textarea class="inp" id="impTxt" rows="8" placeholder="把备份 JSON 粘贴进来" style="font-size:10px;font-family:var(--mono)"></textarea><div style="display:flex;gap:9px;margin-top:12px"><button class="btn ghost" style="flex:1" onclick="closeModal()">取消</button><button class="btn cy" style="flex:1" onclick="doImport()">恢复</button></div>'); }
function doImport(){ const v=$('impTxt').value; if(importJSON(v)){ closeModal(); } }
